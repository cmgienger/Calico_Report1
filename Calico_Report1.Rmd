...
```{r Count number of obs for each lizard; add to dataframe so can be used as a filter}
data <- data %>%
  add_count(LIZARDNUMBER) %>%
  filter(n > 15)
```

```{r Filter by Number of Observations for Each Lizard}
data %>%
  group_by(LIZARDNUMBER) %>%
  count() %>%
  filter(n >= 15) %>%
  arrange(n)
#note that this is just a table to check on the filtering, not a reduction of the dataframe
```
